...
function getUsersFromDatabase(array) {
	...
	// id який дорівнює id користувача,firstName який дорівнює name користувача в верхньому регістрі та years який дорівнює age користувача
...
function createArray(...elements) {
	if (elements.length > 0) {
		...
		const result = [];
		console.log("Помилка: До масиву не передано жодного елементу");
		return result;
	}
...
function copyAndSwapElements(arr, target, start, end) {
	const arrArg = [target, start, end];
	if (
		!arrArg.every(
			(currentValue) =>
				currentValue >= 0 && currentValue <= arr.length - 1
		)
	) {
		return console.log("Неприпустимі індекси");
	}
...
function customFill(arr, value, start = 0, end = arr.length) {
	const arrArg = [start, end];
	if (!Array.isArray(arr)) {
		return false;
	} else if (
		!arrArg.every(
			(currentValue) => currentValue >= 0 && currentValue <= arr.length
		)
	) {
		return false;
	}
